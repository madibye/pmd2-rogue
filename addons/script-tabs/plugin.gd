...
func _update_tab_icons():
	...
	var script_editor: ScriptEditor = Engine.get_singleton(&"EditorInterface").get_script_editor()
	var editor_theme: Theme = Engine.get_singleton(&"EditorInterface").get_editor_theme()
