...
- Create a new submodule in this directory using `git submodule init`, then add our asset repo to this submodule using `git remote add origin -f https://github.com/PMDCollab/RawAsset`. 
    ...
    - This command will take a while to execute since the repo is big, but our end goal is for the Godot not to have to export ~350,000 (at the time of writing!!) spritesheets and other files.
...
- Since the project is very early in development (started November 19, 2025!!), the methods of calling the `fetch_assets.sh` script in-editor are very very subject to change. But at the moment, you can do the following:
    - In a `Unit` resource (these represent Pokemon and will probably have their name changed at some point), pressing the "Fetch Assets" tool button in the editor will fetch the Sprite assets of a Pokemon by their `dex_number`. It gets the sprites for all their forms as well, even though I currently have species with multiple forms separated across multiple `Unit` resources. Should also be 
